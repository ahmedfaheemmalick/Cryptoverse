    ...
    return (
        <div key={coin.uuid} className="w-96 max-w-sm bg-white rounded-lg border border-gray-200 shadow-m py-4">
            <div className="flex flex-col items-center">
                ...
                <h5 className="mb-1 text-xl font-medium text-gray-900 pb-3">{coin.name}</h5>
                ...
                <div className="flex mt-8 space-x-3 lg:mt-6">
                    <Link to={`/coin/${coin.uuid}`} className="inline-flex items-center py-2 px-4 text-sm font-medium text-center text-gray-900 bg-white rounded-lg border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-blue-300">See detail</Link>
                </div>
            </div>
        </div>
    )
